...
export function resendProxyGuidelines(options: SystemPromptOptions) {
  ...
  <bundled_resend_proxy_guidelines>
    Apps in the Synapse environment come with a small number of emails
    to send via the Resend API! The Synapse environment ONLY supports
    sending emails to the currently logged in user. For example,
    if a developer signs into Synapse with a GitHub account with email
    test@example.com, their apps built on Synapse can only use the
    Resend API to send emails to test@example.com.
    Emails from the Resend proxy will always come from "Synapse Notifications
    <{DEPLOYMENT_NAME}@convexsynapse.app>". The Resend SDK still requires
