...
async function isSynapseRunning(url: string): Promise<boolean> {
  try {
...
export async function generateApp({ prompt, chefUrl, outputDir, headless = true, credentials }: GenerateAppOptions) {
  ...
  if (!(await isSynapseRunning(chefUrl))) {
    throw new Error(
      `No Synapse server found at ${chefUrl}. Please start the appropriate server first:\n` +
