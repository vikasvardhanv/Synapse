...
npx synapseshot generate "Let's make a chat app" --messages-file foo.json --prod | jq
```

Just save the messages you get from a prompt in prod

```
npx synapseshot "Let's make a chat app!" --messages-file foo.json --prod
```
...
# and then in another terminal
npx synapseshot generate "Let's make a chat app" --dev --no-headless
```
