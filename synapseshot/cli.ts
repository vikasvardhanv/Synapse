...
const generateCommand = new Command('generate')
  .description('Generate an app using Synapse AI')
  .argument('<prompt>', 'The prompt to send to Synapse')
  // URL group - mutually exclusive options
  .addOption(
    new Option('--synapse-url <url>', 'Custom Synapse URL')
  ...
  .addOption(
    new Option('--prod', 'Use production Synapse at synapse.convex.dev').conflicts(['synapse-url', 'dev', 'local-build']),
  )
  .addOption(
    new Option('--dev', 'Use local dev server at http://127.0.0.1:5173').conflicts(['synapse-url', 'prod', 'local-build']),
  )
  .addOption(
    new Option('--local-build', 'Build (if needed) and run local server at http://localhost:3000')
      .conflicts(['synapse-url', 'prod', 'dev'])
      .default(true),
  )
  ...
  .action(async (prompt: string, options: GenerateOptions) => {
    ...
    if (options.dev) {
      synapseUrl = 'http://127.0.0.1:5173';
    } else if (options.prod) {
      synapseUrl = 'https://synapse.convex.dev';
    } else if (options.chefUrl) {
      synapseUrl = options.chefUrl;
    } else {
      // Default to local-build behavior
      synapseUrl = 'http://localhost:3000';
    }
    log(`Looking for Synapse at ${synapseUrl}`);

    // Check for required environment variables
    const email = process.env.SYNAPSE_EVAL_USER_EMAIL;
    const password = process.env.SYNAPSE_EVAL_USER_PASSWORD;

    if (!email || !password) {
      log('Error: Missing required environment variables');
      log('Please set SYNAPSE_EVAL_USER_EMAIL and SYNAPSE_EVAL_USER_PASSWORD in your .env.local file');
      process.exit(1);
    }
...
const downloadCommand = new Command('download')
  ...
  .action(async (chatUuid: string, options: DownloadOptions) => {
    ...
    const synapseAdminToken = process.env.SYNAPSE_ADMIN_TOKEN;
    if (!synapseAdminToken) {
      log('Error: Missing required environment variables');
      log('Please set SYNAPSE_ADMIN_TOKEN in your .env.local file');
      process.exit(1);
    }
    ...
    const response = await fetch(`${chefSiteUrl}/__debug/download_messages`, {
      ...
      headers: {
        'X-Synapse-Admin-Token': synapseAdminToken,
      },
    });
    if (!response.ok) {
...
program.name('synapseshot').description('Synapse AI CLI').addCommand(generateCommand).addCommand(downloadCommand);
