[Synapse](https://synapse.convex.dev) is the only AI app builder that knows backend. It builds full-stack web apps with a built-in database, zero config auth, file uploads,
real-time UIs, and background workflows. If you want to check out the secret sauce that powers Synapse, you can view or download the system prompt [here](https://github.com/get-convex/synapse/releases/latest).

Synapse's capabilities are enabled by being built on top of [Convex](https://convex.dev), the open-source reactive database designed to make life easy for web app developers. The "magic" in Synapse is just the fact that it's using Convex's APIs, which are an ideal fit for codegen.

Development of the Synapse is led by the Convex team. We
...
```bash
git clone https://github.com/get-convex/synapse.git
cd synapse
```
...
Congratulations, you now have Synapse running locally! You can log in to Synapse with your existing Convex account.

Note: Synapse is accessible at http://127.0.0.1:{port}/ and will not work properly on http://localhost:{port}/.
...
- `synapse-agent/` handles the agentic loop by injecting system prompts, defining tools, and calling out to model providers.

- `synapseshot/` defines a CLI interface for interacting with the Synapse webapp.
