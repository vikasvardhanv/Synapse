...
function formatSynapseTokenPrice(synapseTokens: number): string {
  const rawPrice = (synapseTokens / 1_000_000) * 0.1;
...
function LlmPromptAndResponseView({ promptAndResponse }: { promptAndResponse: LlmPromptAndResponse }) {
  ...
  const synapseTokens = promptAndResponse.synapseTokens || 0;
      ...
      >
        ...
        <div className="flex flex-1 flex-col gap-1">
          ...
          <div className="flex justify-between gap-1 text-xs text-gray-500 dark:text-gray-400">
            ...
            {synapseTokens > 0 && (
              <div>
                <span className="font-semibold text-gray-900 dark:text-gray-100">{formatNumber(synapseTokens)}</span> synapse
                tokens (${formatSynapseTokenPrice(synapseTokens)})
              </div>
            )}
  ...
  data.forEach((item, index) => {
    ...
    if (item.finishReason !== 'tool-calls' || index === data.length - 1) {
      if (currentGroup.length > 0) {
        ...
        const totalSynapseTokens = currentGroup.reduce((sum, item) => {
          return sum + (item.synapseTokens || 0);
        }, 0);
    ...
    <div className="space-y-2 rounded-lg border-2 border-gray-200 p-4 dark:border-gray-700">
      <div className="mb-4 border-b border-gray-200 pb-2 dark:border-gray-700">
        ...
        <div className="mt-1 flex gap-4 text-sm text-gray-500 dark:text-gray-400">
          ...
          <div>
            <span className="font-semibold text-gray-900 dark:text-gray-100">
              {formatNumber(group.summary.totalSynapseTokens)}
            </span>{' '}
            synapse tokens (${formatSynapseTokenPrice(group.summary.totalSynapseTokens)})
          </div>
        </div>
      </div>
...
export default function DebugAllPromptsForChat({ chatInitialId, onClose, isDebugPage }: Props) {
  ...
  const totals = userPromptGroups.reduce(
    (acc, group) => {
      return {
        ...
        synapseTokens: acc.synapseTokens + group.summary.totalSynapseTokens,
      };
    },
    {
      promptTokens: 0,
  ...
  const totalPrice = formatSynapseTokenPrice(totals.synapseTokens);
      ...
      <div className="relative max-h-[90vh] w-[90vw] overflow-auto rounded-lg bg-white p-6 shadow-xl dark:bg-gray-800">
        ...
        <div className="mb-4">
          ...
          <div className="mt-2 flex gap-4 text-sm text-gray-500 dark:text-gray-400">
            ...
            <div>
              <span className="font-semibold text-gray-900 dark:text-gray-100">{formatNumber(totals.synapseTokens)}</span>{' '}
              total synapse tokens (${totalPrice})
            </div>
          </div>
        </div>
